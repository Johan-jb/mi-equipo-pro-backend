    ...
    async generarInforme(jugador, evaluaciones, habilidades) {
        return new Promise((resolve, reject) => {
            try {
                ...
                // Título
                doc.fontSize(20).font('Helvetica-Bold').text('SportMetrics Pro', { align: 'center' });
                ...
                doc.fontSize(10).text(`- DNI: ${jugador.dni || 'No registrado'}`);
                doc.fontSize(10).text(`- Tutor: ${jugador.tutor_nombre || 'No registrado'}`);
                doc.fontSize(10).text(`- Email: ${jugador.tutor_email || 'No registrado'}`);
                doc.moveDown();
